...
public abstract class StructureGeneratorBase extends WorldGenerator
{
	...
	public static final boolean registerCustomBlockRotation(int blockID, ROTATION rotationType, boolean override)
	{
		if (Block.getBlockById(blockID) == null || blockID < 0 || blockID > 4095) {
			throw new IllegalArgumentException("[STRUCTURE GEN API] Error setting custom block rotation for block ID " + blockID + (Block.getBlockById(blockID) == null ? "; block was not found in Block.blocksList. Please register your block." : "Valid ids are (0-4095)"));
		}
	...
	public static final boolean setEntityInStructure(World world, Entity entity, int x, int y, int z)
	{
		...
		while (entity.isEntityInsideOpaqueBlock() && i < iMax)
		{
			...
			LogHelper.log(Level.TRACE, "Entity inside opaque block at " + entity.posX + "/" + entity.posY + "/" + entity.posZ);
	...
	public static final boolean spawnEntityInStructure(World world, Entity entity, int x, int y, int z)
	{
		...
		LogHelper.log(Level.TRACE, "Spawned entity at " + entity.posX + "/" + entity.posY + "/" + entity.posZ);
	...
	public final void setPlayerFacing(Entity entity) {
		if (entity == null) LogHelper.log(Level.WARN, "Null Pointer Exception! Cannot set facing from a null entity.");
	...
	public final void setStructure(Structure structure)
	{
		...
		else LogHelper.log(Level.ERROR, "NULL Structure cannot be set!");
	}
		...
		for (BlockData block : postGenBlocks)
		{
					...
					&& !world.getBlock(block.getPosX(), block.getPosY(), block.getPosZ()).getMaterial().blocksMovement()))
			{
				...
				if (world.getBlockMetadata(block.getPosX(), block.getPosY(), block.getPosZ()) != block.getMetaData()) {
					LogHelper.log(Level.WARN, "Mismatched metadata. Meta from world: " + world.getBlockMetadata(block.getPosX(), block.getPosY(), block.getPosZ()) + ", original: " + block.getMetaData());
				}
