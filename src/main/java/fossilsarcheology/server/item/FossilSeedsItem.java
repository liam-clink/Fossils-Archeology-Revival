...

{
    ...
    public boolean onItemUse(ItemStack stack, EntityPlayer player, World world, BlockPos pos, int i, float a, float b, float c) {
        if (!isFossil && player.canPlayerEdit(pos, i, stack) && player.canPlayerEdit(x, y + 1, z, i, stack)) {
            if (Blocks.sapling.canBlockStay(world, pos) && world.isAirBlock(x, y + 1, z) && world.getBlock(pos) != FABlockRegistry.INSTANCE.welwitschia) {
