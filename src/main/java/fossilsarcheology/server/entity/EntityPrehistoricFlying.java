...
public abstract class EntityPrehistoricFlying extends EntityPrehistoric {
    ...
    public void setFlying(boolean flying) {
        ...
        if (!worldObj.isRemote) {
            if (onGround && flying && !isFlying) {
                this.motionY += 0.35F;
            }
            currentTarget = new ChunkCoordinates((int) this.posX + getRNG().nextInt(10) - 5, (int) this.posY + 10, (int) this.posZ + getRNG().nextInt(10) - 5);
    ...
    public void flyTowardsTarget() {
        if (currentTarget != null && isTargetInAir() && this.isFlying() && this.getDistanceSquared(Vec3.createVectorHelper(currentTarget.posX, this.posY, currentTarget.posZ)) > 3) {
            ...
            motionY += (Math.signum(targetY) * 1.0D - motionY) * 0.100000000372529 * getFlySpeed();
    ...
    protected boolean isTargetInAir() {
        return currentTarget != null && (worldObj.getBlock(currentTarget.posX, currentTarget.posY - 2, currentTarget.posZ).getMaterial() == Material.air && worldObj.getBlock(currentTarget.posX, currentTarget.posY - 1, currentTarget.posZ).getMaterial() == Material.air && worldObj.getBlock(currentTarget.posX, currentTarget.posY, currentTarget.posZ).getMaterial() == Material.air && worldObj.getBlock(currentTarget.posX, currentTarget.posY + 1, currentTarget.posZ).getMaterial() == Material.air);
    }
