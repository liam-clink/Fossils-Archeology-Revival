...
public class DinoAIFeeder extends EntityAIBase {
	...
	public boolean shouldExecute() {
		...
		TileEntityFeeder targetFeeder = this.dinosaur.getNearestFeeder(range / 2);
		if (targetFeeder != null) {
			this.destX = targetFeeder.xCoord;
			this.destY = targetFeeder.yCoord;
			this.destZ = targetFeeder.zCoord;
	...
	public void updateTask() {
		...
		if (this.typeofTarget == FEEDER) {
			...
			if (distance < range) {
				this.dinosaur.getNavigator().tryMoveToXYZ(this.destX, this.destY, this.destZ, 1D);

				if (distance < 2.5D) {
					if (this.targetFeeder != null) {
						if (this.ticksEating < 30) {
							this.ticksEating++;
