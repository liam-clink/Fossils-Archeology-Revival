                ...
                if (hasEquipped(rider, FAItemRegistry.WHIP) || (this.lastTimeSeenWhip < FollowTimeWithoutWhip && this.lastTimeSeenWhip != -1)) {
                    ...
                    if (speedPlayer > 0) {
                        if (!prehistoric.shouldDismountInWater(rider) && prehistoric.isInWater()) {
                            prehistoric.motionX = -MathHelper.sin(rider.rotationYaw / 180.0F * (float) Math.PI) * MathHelper.cos(rider.rotationPitch / 180.0F * (float) Math.PI);
                            prehistoric.motionZ = MathHelper.cos(rider.rotationYaw / 180.0F * (float) Math.PI) * MathHelper.cos(rider.rotationPitch / 180.0F * (float) Math.PI);
                        } else {
