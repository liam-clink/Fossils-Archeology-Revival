...
public class FAWorldGenerator implements IWorldGenerator {
    ...
    public void generate(Random random, int chunkX, int chunkZ, World world, IChunkGenerator chunkGenerator, IChunkProvider chunkProvider) {
        ...
        for (int oreGenDim : Revival.CONFIG_OPTIONS.oreGenerationDimensions) {
            if (dimensionID == oreGenDim) {
        ...
        if (canStructureSpawnInDim(dimensionID)) {
        ...
        if (Revival.CONFIG_OPTIONS.generateHellShips && dimensionID == -1) {
        ...
        if (Revival.CONFIG_OPTIONS.generateTarSites && canStructureSpawnInDim(dimensionID) && random.nextInt(Math.max(Revival.CONFIG_OPTIONS.generateTarSiteRarity, 1)) == 0 && world.provider.hasSkyLight() && !world.provider.isNether() && biome.topBlock.getBlock() == Blocks.GRASS && StructureUtils.canGenOnBlock(world, height)) {
            if (StructureUtils.generateStructureAtWithRandomRotation(TAR_SITE, world, height.down(3), random, true, false)) {
                ...
                villager.setLocationAndAngles(tentPos.getX() - 0.5D, world.getHeight(tentPos).getY() + 1.0D, tentPos.getZ() - 0.5D, 0, 0);
                world.spawnEntity(villager);
            }
        }

        if (Revival.CONFIG_OPTIONS.generateFossilSites && canStructureSpawnInDim(dimensionID) && random.nextInt(Math.max(Revival.CONFIG_OPTIONS.generateFossilSiteRarity, 1)) == 0 && world.provider.hasSkyLight() && !world.provider.isNether() && biome.topBlock.getBlock() == Blocks.GRASS && StructureUtils.canGenOnBlock(world, height)) {
        ...
        if (Revival.CONFIG_OPTIONS.generateAztecWeaponShops && canStructureSpawnInDim(dimensionID) && random.nextInt(Math.max(Revival.CONFIG_OPTIONS.generateWeaponShopRarity, 1)) == 0 && world.provider.hasSkyLight() && !world.provider.isNether() && BiomeDictionary.hasType(biome, BiomeDictionary.Type.JUNGLE) && StructureUtils.canGenOnBlock(world, height)) {
        ...
        if (Revival.CONFIG_OPTIONS.generateTemple && canStructureSpawnInDim(dimensionID) && random.nextInt(Math.max(Revival.CONFIG_OPTIONS.generateTempleRarity, 1)) == 0 && world.provider.hasSkyLight() && !world.provider.isNether() && BiomeDictionary.hasType(biome, BiomeDictionary.Type.JUNGLE) && StructureUtils.canGenOnBlock(world, height)) {
        ...
        if (Revival.CONFIG_OPTIONS.generateAcademy && canStructureSpawnInDim(dimensionID) && random.nextInt(Math.max(Revival.CONFIG_OPTIONS.generateAcademyRarity, 1)) == 0 && world.provider.hasSkyLight() && !world.provider.isNether() && BiomeDictionary.hasType(biome, BiomeDictionary.Type.SANDY) && BiomeDictionary.hasType(biome, BiomeDictionary.Type.DRY) && BiomeDictionary.hasType(biome, BiomeDictionary.Type.HOT) && StructureUtils.canGenOnBlock(world, height)) {
