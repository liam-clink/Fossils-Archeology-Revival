...
IEntityAdditionalSpawnData {
					...
					for (int b=(int)Math.round(this.boundingBox.minY) + 1;(b<=(int)Math.round(this.boundingBox.maxY)+3) &&( b<=127);b++){
						for (int c=(int)Math.round(this.boundingBox.minZ)-1;c<=(int)Math.round(this.boundingBox.maxZ)+1;c++){
							...
							if(!(block instanceof BlockBush) && !(block instanceof BlockDoublePlant) && !(block instanceof BlockLiquid) &&block != Blocks.bedrock &&
									block != Fossil.ancientGlass && block != Fossil.strongGlass && block.getBlockHardness(worldObj, a, b, c) < hardness){
								this.motionX *= 0.6D;
								this.motionZ *= 0.6D;
	...
	protected String getModelTexture() {
		return Fossil.modid + ":" + "textures/mob/DinosaurModels/DinoModel"
	...
	protected String getLivingSound() {
		if (this.isModelized())
			return null;
		return Fossil.modid + ":" + this.SelfType.toString().toLowerCase()
				+ "_living";
	}
	...
	public void ShowPedia(GuiPedia p0) {
		...
		p0.reset();
		p0.PrintPictXY(new ResourceLocation(Fossil.modid + ":"
	...
	public void SendStatusMessage(EnumSituation var1) {
...

		{
			...
			String Status2 = StatCollector.translateToLocal("status."
					+ var1.toString());
			Fossil.ShowMessage(Status1 + Dino + " " + Status2,
			...
			if (itemstack != null) {
				...
				} else// no food, but not nothing
				{
					...
					if (FMLCommonHandler.instance().getSide().isClient()
							&& itemstack.getItem() == Fossil.dinoPedia) {
						// DINOPEDIA
						// EntityDinosaur.pediaingDino = this;
						this.setPedia();
						player.openGui(Fossil.instance/* player */, 4,
								this.worldObj, (int) this.posX,
								(int) this.posY, (int) this.posZ);
						return true;
					}

					if (itemstack.getItem() == Fossil.whip && this.isTamed()
	 ...
	 * var1.itemID == Fossil.whip.itemID; }
	 */
