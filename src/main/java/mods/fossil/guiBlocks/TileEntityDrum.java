...
public class TileEntityDrum extends TileEntity {
	 ...
	 * 
	 * var1.addBlockEvent(var2, var3, var4, Fossil.drum.blockID, var6,
	 * this.note); } }
	 */

	/*
	 * private String GetOrderString() { return Fossil.GetLangTextByKey("Order."
	 * + this.Order.toString()); }
	 */
	...
	public boolean SendOrder(Item item, EntityPlayer var2) {
		 ...
		 * Fossil.GetLangTextByKey("Drum.Msg.Head"); String var6 =
		 * Fossil.GetLangTextByKey("Drum.Msg.Middle"); String var7 =
		 * Fossil.GetLangTextByKey("Drum.Msg.Tail");
		 */
		this.worldObj.playSoundEffect((double) this.xCoord,
				(double) this.yCoord, (double) this.zCoord,
		...
		if (item != Fossil.skullStick) // That is treated specially ;)
		{
			...
			while (it.hasNext()) {
				...
				/*
				 * Fossil.ShowMessage("YES",var2); } else
				 * Fossil.ShowMessage("NOPE",var2);
				 */
			}
		} else {
			Fossil.ShowMessage(StatCollector
		 ...
		 * Fossil.skullStick.itemID && var1 != Item.arrow.itemID) { return
		 ...
		 * Fossil.GetLangTextByKey("Dino.Raptor"); }
		 * 
		 * if (var1 == Item.arrow.itemID) { this.OrderPTS(); var3 =
		 * Fossil.GetLangTextByKey("Dino.Pterosaur"); }
		 * 
		 * if (var1 == Fossil.skullStick.itemID) { this.OrderTRex(var2); }
		 * 
		 * var4 = this.GetOrderString();
		 * 
		 * if (var1 != Fossil.skullStick.itemID) { Fossil.ShowMessage(var5 +
		 * var3 + var6 + var4 + var7, var2); return true; } else { String var8 =
		 * Fossil.GetLangTextByKey("Drum.Msg.TRex." +
		 * String.valueOf(this.Order.ordinal()/*ToInt() + 1*));
		 * Fossil.ShowMessage(var8, var2); return true; } }
		 */
	}
