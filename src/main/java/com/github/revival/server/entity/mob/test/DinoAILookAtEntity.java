...
public class DinoAILookAtEntity extends EntityAIBase {
	...
	public DinoAILookAtEntity(EntityLiving entity, Class classy, float i) {
		this.theWatcher = entity;
		this.watchedClass = classy;
		this.maxDistanceForPlayer = i;
	...
	public boolean shouldExecute() {
		...
		if (this.theWatcher instanceof EntityFlyingPrehistoric && ((EntityFlyingPrehistoric) this.theWatcher).isFlying()) {
			return false;
		}
		...
		if (this.theWatcher.getRNG().nextFloat() >= this.field_75331_e) {
			...
			if (this.watchedClass == EntityPlayer.class) {
				...
				if (closestEntity != null) {
					if (closestEntity.boundingBox.maxX * 1.5F < theWatcher.boundingBox.maxX && closestEntity.boundingBox.minX * 1.5F > theWatcher.boundingBox.minX && closestEntity.boundingBox.minZ * 1.5F < theWatcher.boundingBox.minZ && closestEntity.boundingBox.minZ * 1.5F > theWatcher.boundingBox.minZ) {
						return false;
					}
				}
			}
