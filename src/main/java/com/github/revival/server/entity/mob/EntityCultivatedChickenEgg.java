...
public class EntityCultivatedChickenEgg extends EntityThrowable {
    ...
    /**
     * else{ if (!this.worldObj.isRemote && this.rand.nextInt(8) == 0) { byte b0
     * = 1;
     * <p/>
     * if (this.rand.nextInt(32) == 0) { b0 = 4; }
     * <p/>
     * for (int i = 0; i < b0; ++i) { this.spawnAnimal();
     * <p/>
     * } for (int j = 0; j < 8; ++j) {
     * this.worldObj.spawnParticle("snowballpoof", this.posX, this.posY,
     * this.posZ, 0.0D, 0.0D, 0.0D); } } } Called when this EntityThrowable hits
     * a block or entity.
     */
    @Override
