...
public class GallimimusEntity extends NewPrehistoricEntity {
	...
	public DinosaurEntity flockLeader = null;
	public boolean isTamed = false;
	private ChunkCoordinates currentHerdTarget;
	public Flock flock;

	public GallimimusEntity(World var1)
	{
		...
		this.tasks.addTask(2, new EntityAIAvoidEntity(this, TyrannosaurusEntity.class, 16.0F, 0.8D, 1.33D));
		this.tasks.addTask(2, new EntityAIAvoidEntity(this, SpinosaurusEntity.class, 16.0F, 0.8D, 1.33D));
		this.tasks.addTask(2, new EntityAIAvoidEntity(this, DilophosaurusEntity.class, 16.0F, 0.8D, 1.33D));
		this.tasks.addTask(2, new EntityAIAvoidEntity(this, DeinonychusEntity.class, 16.0F, 0.8D, 1.33D));
		this.tasks.addTask(2, new EntityAIAvoidEntity(this, MosasaurusEntity.class, 16.0F, 0.8D, 1.33D));
		this.tasks.addTask(2, new EntityAIAvoidEntity(this, LiopleurodonEntity.class, 16.0F, 0.8D, 1.33D));
		this.tasks.addTask(2, new EntityAIAvoidEntity(this, BrachiosaurusEntity.class, 4.0F, 0.8D, 1.33D));
	}
	...
	public GallimimusEntity spawnBabyAnimal(EntityAnimal var1)
	{
		return new GallimimusEntity(this.worldObj);
	}
	...
	public void onLivingUpdate(){
		...
		if(closestEntity() != null && ((GallimimusEntity)closestEntity()).flock != null){
			((GallimimusEntity)closestEntity()).flock.flockMembers.add(this);
		}else{
			if(flock == null){
				flock = new Flock();
				...
				flock.onUpdate();
				if(this.closestEntity() instanceof GallimimusEntity && ((GallimimusEntity)this.closestEntity()).flock != this.flock){
	...
	public DinosaurEntity closestEntity(){
		...
		for(Entity mob: entities){
			if(mob instanceof DinosaurEntity){
				if(((DinosaurEntity)mob).selfType == this.selfType){
					return (DinosaurEntity) mob;
				}
			}
		}
		return null;
	...
	{
		GallimimusEntity baby = new GallimimusEntity(this.worldObj);
