...
public class AnalyzerBlock extends BlockContainer {
    ...
    public void onBlockPlacedBy(World world, int x, int y, int z, EntityLivingBase placer, ItemStack stack) {
        ...
        if (stack.hasDisplayName()) {
            ((AnalyzerTile) world.getTileEntity(x, y, z)).setGuiDisplayName(stack.getDisplayName());
        }
    }
    ...
    public void breakBlock(World world, int x, int y, int z, Block block, int var6) {
        if (!keepFurnaceInventory) {
            AnalyzerTile tileentity = (AnalyzerTile) world.getTileEntity(x, y, z);
