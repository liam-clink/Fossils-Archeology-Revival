...
import com.github.revival.server.entity.mob.FailuresaurusEntity;
...
public class CultivateBlock extends BlockContainer {
    ...
    private void returnDNA(World world, int x, int y, int z) {
        if (!keepFurnaceInventory) {
            CultivateTile tile = (CultivateTile) world.getTileEntity(x, y, z);
            if (tile != null) {
    ...
    public void onBlockRemovalLost(World world, int x, int y, int z, boolean isActive) {
        ...
        if (!world.isRemote) {
            if (isActive) {
                CultivateTile tileentity = (CultivateTile) world.getTileEntity(x, y, z);
                if (tileentity != null) {
                    ...
                    if (tileentity.getDNAType() == 2 || tileentity.getDNAType() == 3) {
                        ...
                        if (rand >= 10) {
                            creature = new FailuresaurusEntity(world);
                            ((FailuresaurusEntity) creature).setSkin(new Random()
                                    .nextInt(3));
                        }
