...
import com.github.revival.server.block.CultivateBlock;
...
public class CultivateTile extends TileEntity implements IInventory,
        ISidedInventory {
    ...
    public void updateEntity() {
        ...
        if (!this.worldObj.isRemote) {
            ...
            if (var1 != this.furnaceCookTime > 0) {
                var2 = true;
                CultivateBlock.updateFurnaceBlockState(
        ...
        if (this.furnaceCookTime == 3001 && (new Random()).nextInt(100) < 20) {
            ((CultivateBlock) FABlockRegistry.blockcultivateIdle).onBlockRemovalLost(this.worldObj, this.xCoord, this.yCoord, this.zCoord, true);
        }
    }
       ...
       /* if (itemstack.getItem() == FAItemRegistry.dnaTerrorBird)
        {
            return new ItemStack(FAItemRegistry.cultivatedTerrorBirdEgg, 1,
                    new Random().nextInt(TerrorBirdEntity.names.length));
        }*/
