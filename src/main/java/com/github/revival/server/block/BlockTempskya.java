...
public class BlockTempskya extends BlockBush {
	...
	public boolean canPlaceBlockAt(World world, int x, int y, int z) {
		if (world.getBlock(x, y, z) != Blocks.air && world.getBlock(x, y + 1, z) != Blocks.air && world.getBlock(x, y + 2, z) != Blocks.air && world.getBlock(x, y + 3, z) != Blocks.air) {
			return false;
		} else {
	...
	public AxisAlignedBB getCollisionBoundingBoxFromPool(World par1World, int par2, int par3, int par4) {
		...
		float f2 = 0.375F;
		float f3 = 0.625F;
		return AxisAlignedBB.getBoundingBox((double) par2 + f, (double) par3 + f2, (double) par4, (double) par2 + f1, (double) ((float) par3 + this.maxY), (double) par4 + f3);
	}
